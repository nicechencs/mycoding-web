...
describe('useNavigationHandler Hook', () => {
  ...
  describe('基础功能测试', () => {
    it('应该返回导航处理器函数', () => {
      const { result } = renderHook(() =>
        useNavigationHandler({ platform: 'desktop' })
      )
  ...
  describe('桌面端导航测试', () => {
    ...
    it('应该为活动路由生成正确的类名', () => {
      ...
      expect(className).toBe(
        'px-3 py-2 text-sm font-medium bg-gray-900 text-white'
      )
    })
  ...
  describe('移动端导航测试', () => {
    ...
    it('应该为移动端活动路由生成正确的类名', () => {
      const { result } = renderHook(() =>
        useNavigationHandler({ platform: 'mobile' })
      )
      ...
      expect(className).toBe(
        'block px-3 py-2 text-base font-medium bg-gray-900 text-white'
      )
    })
  ...
  describe('边界条件测试', () => {
    ...
    it('应该处理特殊字符路径', () => {
      ...
      expect(result.current.isActiveRoute('/resources')).toBe(true)
      expect(result.current.isActiveRoute('/resources?category=web-dev')).toBe(
        true
      )
    })
