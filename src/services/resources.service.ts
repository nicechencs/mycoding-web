...
export interface IResourcesService {
  ...
  getResourcesByCategory(category: string, params?: QueryParams): Promise<ApiResponse<Resource[]>>;
  getResourceComments(resourceId: string, params?: QueryParams): Promise<ApiResponse<ResourceComment[]>>;
  getResourceRatings(resourceId: string): Promise<ApiResponse<ResourceRating[]>>;
  getResourceRatingDistribution(resourceId: string): Promise<ApiResponse<ResourceRatingDistribution>>;
  getRelatedResources(resourceId: string, limit?: number): Promise<ApiResponse<Resource[]>>;
...
export class ResourcesService implements IResourcesService {
   ...
   * 获取资源列表（支持分页、搜索、筛选）
   */
  async getResources(params: QueryParams = {}): Promise<PaginatedResponse<Resource>> {
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Resource[]>('/resources', { params });
      ...
      if (response.success && response.data) {
        const result: PaginatedResponse<Resource> = {
          ...
          pagination: {
            ...
            hasNext: (params.page || 1) * (params.limit || 12) < response.data.length,
  ...
  async getResourceBySlug(slug: string): Promise<ApiResponse<Resource>> {
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Resource>(`/resources/slug/${slug}`);
      this.cache.set(cacheKey, response);
      return response;
    }
  }
   ...
   * 获取精选资源
   */
  async getFeaturedResources(limit: number = 6): Promise<ApiResponse<Resource[]>> {
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Resource[]>('/resources/featured', {
        params: { limit },
      });
   ...
   * 根据分类获取资源
   */
  async getResourcesByCategory(category: string, params: QueryParams = {}): Promise<ApiResponse<Resource[]>> {
    const cacheKey = CacheManager.generateKey('resources:category', { category, ...params });
    ...
    if (isDevelopment()) {
      ...
      if (params.filters?.rating) {
        resources = resources.filter(resource => resource.rating >= (params.filters?.rating || 0));
      }

      // 应用标签过滤
      if (params.filters?.tags && params.filters.tags.length > 0) {
        resources = resources.filter(resource => 
          params.filters?.tags?.some((tag: string) => resource.tags.includes(tag)) || false
        );
      }
      ...
      const response = await this.apiClient.get<Resource[]>(`/resources/category/${category}`, {
        params,
      });
   ...
   * 获取资源评论
   */
  async getResourceComments(resourceId: string, params: QueryParams = {}): Promise<ApiResponse<ResourceComment[]>> {
    const cacheKey = CacheManager.generateKey('resources:comments', { resourceId, ...params });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<ResourceComment[]>(`/resources/${resourceId}/comments`, {
        params,
      });
   ...
   * 获取资源评分
   */
  async getResourceRatings(resourceId: string): Promise<ApiResponse<ResourceRating[]>> {
    const cacheKey = CacheManager.generateKey('resources:ratings', { resourceId });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<ResourceRating[]>(`/resources/${resourceId}/ratings`);
   ...
   * 获取资源评分分布
   */
  async getResourceRatingDistribution(resourceId: string): Promise<ApiResponse<ResourceRatingDistribution>> {
    const cacheKey = CacheManager.generateKey('resources:rating-distribution', { resourceId });
    
    const cached = this.cache.get<ApiResponse<ResourceRatingDistribution>>(cacheKey);
    if (cached) {
      return cached;
    }
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<ResourceRatingDistribution>(`/resources/${resourceId}/rating-distribution`);
   ...
   * 获取相关资源
   */
  async getRelatedResources(resourceId: string, limit: number = 4): Promise<ApiResponse<Resource[]>> {
    const cacheKey = CacheManager.generateKey('resources:related', { resourceId, limit });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Resource[]>(`/resources/${resourceId}/related`, {
        params: { limit },
      });
   ...
   * 搜索资源
   */
  async searchResources(query: string, params: QueryParams = {}): Promise<ApiResponse<Resource[]>> {
    const cacheKey = CacheManager.generateKey('resources:search', { query, ...params });
    ...
    if (isDevelopment()) {
      ...
      let resources = mockResources.filter(resource => 
        resource.title.toLowerCase().includes(searchLower) ||
        resource.description.toLowerCase().includes(searchLower) ||
        (resource.detailedDescription && resource.detailedDescription.toLowerCase().includes(searchLower)) ||
        resource.tags.some(tag => tag.toLowerCase().includes(searchLower)) ||
      ...
      const response = await this.apiClient.get<Resource[]>('/resources/search', {
  ...
  private mockGetResources(params: QueryParams): PaginatedResponse<Resource> {
    ...
    if (params.filters?.category && params.filters.category !== 'all') {
      resources = resources.filter(resource => resource.category === params.filters?.category);
    }
