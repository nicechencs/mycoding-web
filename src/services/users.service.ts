...
export interface IUsersService {
  ...
  updateUserPreferences(id: string, preferences: Partial<UserPreferences>): Promise<ApiResponse<UserPreferences>>;
  ...
  getFollowers(userId: string, params?: QueryParams): Promise<ApiResponse<User[]>>;
  getFollowing(userId: string, params?: QueryParams): Promise<ApiResponse<User[]>>;
...
export class UsersService implements IUsersService {
  ...
  async getUserByEmail(email: string): Promise<ApiResponse<User>> {
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<User>(`/users/by-email/${encodeURIComponent(email)}`);
      this.cache.set(cacheKey, response);
      return response;
    }
  }
  ...
  async getUserPreferences(id: string): Promise<ApiResponse<UserPreferences>> {
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<UserPreferences>(`/users/${id}/preferences`);
      this.cache.set(cacheKey, response);
      return response;
    }
  }
   ...
   * 更新用户偏好设置
   */
  async updateUserPreferences(id: string, preferences: Partial<UserPreferences>): Promise<ApiResponse<UserPreferences>> {
    if (isDevelopment()) {
      // 获取当前偏好设置
      ...
      const response = await this.apiClient.patch<UserPreferences>(`/users/${id}/preferences`, preferences);
   ...
   * 搜索用户
   */
  async searchUsers(query: string, params: QueryParams = {}): Promise<ApiResponse<User[]>> {
    const cacheKey = CacheManager.generateKey('users:search', { query, ...params });
  ...
  async followUser(userId: string): Promise<ApiResponse<boolean>> {
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.post<boolean>(`/users/${userId}/follow`);
  ...
  async unfollowUser(userId: string): Promise<ApiResponse<boolean>> {
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.delete<boolean>(`/users/${userId}/follow`);
   ...
   * 获取用户粉丝列表
   */
  async getFollowers(userId: string, params: QueryParams = {}): Promise<ApiResponse<User[]>> {
    const cacheKey = CacheManager.generateKey('users:followers', { userId, ...params });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<User[]>(`/users/${userId}/followers`, { params });
      this.cache.set(cacheKey, response);
      return response;
    }
  }
   ...
   * 获取用户关注列表
   */
  async getFollowing(userId: string, params: QueryParams = {}): Promise<ApiResponse<User[]>> {
    const cacheKey = CacheManager.generateKey('users:following', { userId, ...params });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<User[]>(`/users/${userId}/following`, { params });
      this.cache.set(cacheKey, response);
      return response;
    }
  }
  ...
  async isFollowing(userId: string): Promise<ApiResponse<boolean>> {
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<boolean>(`/users/${userId}/is-following`);
  ...
  private clearFollowCache(userId: string): void {
    this.cache.delete(CacheManager.generateKey('users:is-following', { userId }));
