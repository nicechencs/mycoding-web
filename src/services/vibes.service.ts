...
export interface IVibesService {
  ...
  getVibesByUser(userId: string, params?: QueryParams): Promise<ApiResponse<Vibe[]>>;
  getVibeComments(vibeId: string, params?: QueryParams): Promise<ApiResponse<VibeComment[]>>;
...
export class VibesService implements IVibesService {
   ...
   * 获取指定用户的动态
   */
  async getVibesByUser(userId: string, params: QueryParams = {}): Promise<ApiResponse<Vibe[]>> {
    const cacheKey = CacheManager.generateKey('vibes:user', { userId, ...params });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Vibe[]>(`/vibes/user/${userId}`, {
        params,
      });
   ...
   * 获取动态评论
   */
  async getVibeComments(vibeId: string, params: QueryParams = {}): Promise<ApiResponse<VibeComment[]>> {
    const cacheKey = CacheManager.generateKey('vibes:comments', { vibeId, ...params });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<VibeComment[]>(`/vibes/${vibeId}/comments`, {
        params,
      });
   ...
   * 获取相关动态
   */
  async getRelatedVibes(vibeId: string, limit: number = 5): Promise<ApiResponse<Vibe[]>> {
    const cacheKey = CacheManager.generateKey('vibes:related', { vibeId, limit });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Vibe[]>(`/vibes/${vibeId}/related`, {
        params: { limit },
      });
   ...
   * 搜索动态
   */
  async searchVibes(query: string, params: QueryParams = {}): Promise<ApiResponse<Vibe[]>> {
    const cacheKey = CacheManager.generateKey('vibes:search', { query, ...params });
   ...
   * 根据标签获取动态
   */
  async getVibesByTag(tag: string, params: QueryParams = {}): Promise<ApiResponse<Vibe[]>> {
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Vibe[]>(`/vibes/tag/${encodeURIComponent(tag)}`, {
        params,
      });
  ...
  async getTrendingTags(): Promise<ApiResponse<string[]>> {
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<string[]>('/vibes/trending-tags');
  ...
  private calculateRelevance(vibe: Vibe, searchQuery: string): number {
    ...
    // 内容匹配（权重最高）
    const contentMatches = (vibe.content.toLowerCase().match(new RegExp(searchQuery, 'g')) || []).length;
    score += contentMatches * 3;
    
    // 标签完全匹配
    const exactTagMatches = vibe.tags.filter(tag => tag.toLowerCase() === searchQuery).length;
    score += exactTagMatches * 2;
    
    // 标签部分匹配
    const partialTagMatches = vibe.tags.filter(tag => 
      tag.toLowerCase().includes(searchQuery) && tag.toLowerCase() !== searchQuery
    ).length;
