...
export interface IArticlesService {
  ...
  getArticlesByCategory(category: string, params?: QueryParams): Promise<ApiResponse<Article[]>>;
  getLatestArticles(limit?: number): Promise<ApiResponse<Article[]>>;
  getArticleComments(articleId: string, params?: QueryParams): Promise<ApiResponse<Comment[]>>;
  getRelatedArticles(slug: string, limit?: number): Promise<ApiResponse<Article[]>>;
  searchArticles(query: string, params?: QueryParams): Promise<ApiResponse<Article[]>>;
}
...
export class ArticlesService implements IArticlesService {
   ...
   * 获取文章列表（支持分页、搜索、筛选）
   */
  async getArticles(params: QueryParams = {}): Promise<PaginatedResponse<Article>> {
    ...
    if (isDevelopment()) {
      ...
      // 生产模式：调用API
      const response = await this.apiClient.get<Article[]>('/articles', { params });
      ...
      if (response.success && response.data) {
        const result: PaginatedResponse<Article> = {
          ...
          pagination: {
            ...
            hasNext: (params.page || 1) * (params.limit || 10) < response.data.length,
  ...
  async getFeaturedArticles(limit: number = 6): Promise<ApiResponse<Article[]>> {
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Article[]>('/articles/featured', {
        params: { limit },
      });
   ...
   * 根据分类获取文章
   */
  async getArticlesByCategory(category: string, params: QueryParams = {}): Promise<ApiResponse<Article[]>> {
    const cacheKey = CacheManager.generateKey('articles:category', { category, ...params });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Article[]>(`/articles/category/${category}`, {
        params,
      });
   ...
   * 获取文章评论
   */
  async getArticleComments(articleId: string, params: QueryParams = {}): Promise<ApiResponse<Comment[]>> {
    const cacheKey = CacheManager.generateKey('articles:comments', { articleId, ...params });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Comment[]>(`/articles/${articleId}/comments`, {
        params,
      });
   ...
   * 获取相关文章
   */
  async getRelatedArticles(slug: string, limit: number = 3): Promise<ApiResponse<Article[]>> {
    const cacheKey = CacheManager.generateKey('articles:related', { slug, limit });
    ...
    if (isDevelopment()) {
      ...
      const response = await this.apiClient.get<Article[]>(`/articles/${slug}/related`, {
        params: { limit },
      });
   ...
   * 搜索文章
   */
  async searchArticles(query: string, params: QueryParams = {}): Promise<ApiResponse<Article[]>> {
    const cacheKey = CacheManager.generateKey('articles:search', { query, ...params });
  ...
  private mockGetArticles(params: QueryParams): PaginatedResponse<Article> {
    ...
    if (params.filters?.category && params.filters.category !== 'all') {
      articles = articles.filter(article => article.category === params.filters?.category);
    }
