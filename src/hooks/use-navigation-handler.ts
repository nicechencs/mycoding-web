...
export function useNavigationHandler(options: NavigationHandlerOptions) {
  ...
  const handleNavigation = useCallback(
    (item: NavigationItem, event: React.MouseEvent) => {
      // 统一的导航日志记录
      console.log(
        `${platform.charAt(0).toUpperCase() + platform.slice(1)} navigation clicked:`,
        {
          title: item.title,
          href: item.href,
  ...
  const getNavigationClassName = useCallback(
    (href: string, baseClasses: string, activeClasses: string) => {
