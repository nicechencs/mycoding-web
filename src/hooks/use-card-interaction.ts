...
interface UseCardInteractionOptions {
   ...
   * 点击前的回调函数
   */
  beforeNavigate?: (item: BaseCardItem, event: React.MouseEvent) => boolean | void
...
export function useCardInteraction<T extends BaseCardItem>(
   ...
   * 检查点击目标是否应该被阻止
   */
  const shouldPreventClick = useCallback((target: HTMLElement): boolean => {
   ...
   * 生成路由路径
   */
  const generateRoute = useCallback((item: T): string => {
   ...
   * 记录交互日志
   */
  const logInteraction = useCallback((item: T, route: string, event: React.MouseEvent) => {
    if (enableDebugLog) {
  ...
  const handleCardClick = useCallback((item: T) => {
    return (event: React.MouseEvent<HTMLElement>) => {
      ...
      try {
        ...
        if (enableDebugLog) {
          console.error(`Card navigation failed for ${cardType}:`, error, item)
        }
   ...
   * 与卡片点击不同，这个会阻止事件冒泡
   */
  const handleActionClick = useCallback((item: T, action?: () => void) => {
    return (event: React.MouseEvent<HTMLElement>) => {
      // 阻止事件冒泡到卡片
   ...
   * 获取卡片的基础交互属性
   */
  const getCardProps = useCallback((item: T) => {
    return {
