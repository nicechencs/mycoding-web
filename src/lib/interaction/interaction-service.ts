...
export class InteractionService {
  ...
  static async rateResource(
    ...
    const rating: Rating = {
      ...
      score: data.score,
      review: data.review,
      createdAt: existingIndex > -1 ? ratings[existingIndex].createdAt : new Date()
    }
  ...
  static async getInteractionStats(
    ...
    const targetLikes = likes.filter(l => l.targetId === targetId && l.targetType === targetType)
    const targetFavorites = favorites.filter(f => f.targetId === targetId && f.targetType === targetType)
