...
// SVG图标组件
const SvgIcon = ({ type, className, filled = false }: { type: string, className?: string, filled?: boolean }) => {
  const iconProps = {
...
// 单个统计项组件
function StatItemComponent({ label, value, icon, color = 'default', onClick, active }: StatItem) {
  ...
  const renderIcon = () => {
    ...
    // SVG图标
    if (icon === 'like') {
      return <SvgIcon type="heart" className={cn('w-5 h-5', active && 'fill-current')} filled={active} />
    }
    ...
    <button
      className={cn(
        ...
        isClickable ? (
          ...
          }[icon as string] || colorMap[color] : colorMap[color]
...
export function CardStats({ 
  ...
  const containerClass = layout === 'justified' 
  ...
  return (
    <div className={cn(containerClass, sizeClass, className)}>
      <div className={cn(
        layout === 'justified' ? 'flex items-center space-x-4' : 'flex items-center space-x-4'
      )}>
...
export function ArticleStats({ 
  ...
  const stats: StatItem[] = [
    { label: '查看', value: viewCount, icon: 'view', color: 'blue', onClick: onView },
    { label: '点赞', value: likeCount, icon: 'like', color: 'red', onClick: onLike },
    { label: '评论', value: commentCount, icon: 'comment', color: 'green', onClick: onComment }
  ]
