  ...
  const renderTitle = () => {
    if (link) {
      if (link.external) {
        ...
        return (
          <Link href={link.href as any} className="block group">
            {titleContent}
          </Link>
        )
      }
...
export function CardFooter({
  ...
  const showOnlyCreated =
    showCreated && createdAt && (!showUpdated || !updatedAt)
  const showOnlyUpdated =
    showUpdated && updatedAt && (!showCreated || !createdAt)
  ...
  return (
    <div className={cn('flex items-center text-xs text-gray-400', className)}>
      ...
      {showOnlyCreated && <span>创建于 {formatDate(createdAt)}</span>}

      {showOnlyUpdated && <span>更新于 {formatDate(updatedAt)}</span>}
    </div>
  )
}
  ...
  const renderPrimaryAction = () => {
    ...
    if (primaryAction.href) {
      if (primaryAction.external) {
        ...
        return (
          <Link
            href={primaryAction.href as any}
            className={cn(baseClasses, variantClasses)}
          >
            {content}
          </Link>
